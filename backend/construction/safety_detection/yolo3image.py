...
def yolo3(path):
    ...
    with open('safety_detection/yolo/class.names') as f:
        # Getting labels reading every line
        # and putting them into the list
        labels = [line.strip() for line in f]

    network = cv2.dnn.readNetFromDarknet('safety_detection/yolo/yolov3_ppe_test.cfg',
                                         'safety_detection/yolo/yolov3_ppe_train_15000.weights')
    ...
    if len(results) > 0:
        ...
        for i in results.flatten():
            ...
            my_dict[f'{counter}'] = f'{labels[int(class_numbers[i])]}'
...
# print("Safety Percent %f" % (counter / 4 * 100))
# print(safety_string)
